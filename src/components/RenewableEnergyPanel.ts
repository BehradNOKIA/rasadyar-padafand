...
export class RenewableEnergyPanel extends Panel {
  constructor() {
    super({ id: 'renewable', title: 'انرژی تجدیدپذیر', trackActivity: false, infoTooltip: t('components.renewable.infoTooltip') });
  }
  ...
  public setData(result: RenewableEnergyFetchResult): void {
    ...
    if (data === null || (data.globalPercentage === 0 && !data.regions?.length)) {
      ...
      empty.textContent = 'داده انرژی تجدیدپذیر موجود نیست';
  ...
  private renderGauge(
    ...
    yearLabel.textContent = `داده‌های سال ${year}`;
  ...
  public setCapacityData(series: CapacitySeries[]): void {
    ...
    header.textContent = 'ظرفیت نصب‌شده آمریکا (EIA)';
  ...
  private renderCapacityChart(
    ...
    const items: Array<{ color: string; label: string }> = [
      { color: solarColor, label: 'خورشیدی' },
      { color: windColor, label: 'بادی' },
      { color: coalColor, label: 'زغال‌سنگ' },
    ];

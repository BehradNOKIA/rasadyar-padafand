...
function renderAssetCard(asset: AssetView, clickTarget: string | null): string {
  ...
  const staleBadge = asset.stale ? ' <span class="pos-badge pos-badge--stale">قدیمی</span>' : '';
  const warmupBadge = asset.warmup ? ' <span class="pos-badge pos-badge--warmup">در حال آماده‌سازی</span>' : '';
  ...
  const title = `${asset.symbol} score ${score}/100` +
    ...
    (asset.oiDelta1h != null ? ` | تغییر سود باز ${oiStr}` : '') +
    (asset.warmup ? ' | در حال آماده‌سازی' : '') +
    (asset.stale ? ' | داده بالادستی قدیمی' : '');
  ...
  return `<div class="pos-card${elevatedClass}${cursorClass}"${glowStyle}${clickAttr} title="${escapeHtml(title)}">
    ...
    <div class="pos-card__metrics">
      <span class="${fundingColor}" title="نرخ تأمین مالی ساعتی">${escapeHtml(fundingStr)}</span>
      <span class="${oiColor}" title="تغییر سود باز در یک ساعت">${escapeHtml(oiStr)}</span>
    </div>
  </div>`;
}

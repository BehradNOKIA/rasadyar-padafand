...
export class ResilienceWidget {
      ...
      h(
        ...
        h(
          'span',
          {
            ...
            title: preview ? 'فقط پیش‌نمایش' : 'سیگنال اطمینان مبتنی بر پوشش داده و برآورد مقادیر مفقود.',
          },
          ...
          return dataVersionLabel
            ? [h(
                'span',
                {
                  ...
                  title: 'تاریخ آخرین به‌روزرسانی بسته داده پایه ثابت. ورودی‌های زنده مانند رویدادهای درگیری، تحریم‌ها و قیمت‌ها ممکن است جدیدتر باشند؛ نشان تازگی هر بُعد را بررسی کنید.',
                },
                dataVersionLabel,
              )]
            : [];
  ...
  private renderDimensionConfidenceGrid(data: ResilienceScoreResponse): HTMLElement {
    ...
    return h(
      'div',
      {
        ...
        title: 'پوشش داده در هر بُعد. برای دیدن درصد پوشش و منشأ مشاهده، نشانگر را روی هر خانه نگه دارید.',
      },
  ...
  private renderDimensionConfidenceCell(dim: DimensionConfidence): HTMLElement {
    ...
    const titleParts: string[] = [
      dim.absent ? `${dim.label}: داده‌ای موجود نیست` : `${dim.label}: ${dim.coveragePct}٪ پوشش، ${dim.status}`,
    ];

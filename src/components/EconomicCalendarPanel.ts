...
function formatDateGroup(dateStr: string): string {
  if (!dateStr || dateStr === 'Unknown') return 'نامشخص';
  const d = new Date(`${dateStr}T00:00:00`);
  if (Number.isNaN(d.getTime())) return dateStr;
  return formatIranDate(d, { weekday: 'short', month: 'short', day: 'numeric' });
}
...
function countdown(dateStr: string): string {
  ...
  if (days < 0) return Math.abs(days) < 14 ? `${Math.abs(days)} روز پیش` : `${Math.round(Math.abs(days) / 7)} هفته پیش`;
  if (days < 14) return `${days} روز دیگر`;
  return `${Math.round(days / 7)} هفته دیگر`;
}
...
export class EconomicCalendarPanel extends Panel {
  ...
  constructor() {
    super({ id: 'economic-calendar', title: 'تقویم اقتصادی', showCount: false, infoTooltip: t('components.economicCalendar.infoTooltip') });
  ...
  public async fetchData(): Promise<boolean> {
    this.showLoading('در حال بارگذاری تقویم اقتصادی...');
    try {
      ...
      if (resp.unavailable || !resp.events || resp.events.length === 0) {
        if (!this._hasData) this.showError('داده‌های تقویم اقتصادی در دسترس نیست.', () => void this.fetchData());
        return false;
      }
      ...
      if (!this._hasData) this.showError('بارگذاری تقویم اقتصادی ناموفق بود.', () => void this.fetchData());
      return false;
    }
  }
  ...
  private _render(): void {
    if (!this._hasData) {
      this.showError('رویداد اقتصادی پیش‌رو وجود ندارد.', () => void this.fetchData());
      return;
    }
    ...
    const emptyMsgText = this._region === 'all'
      ? 'رویداد اقتصادی پیش‌رو وجود ندارد'
      : 'برای منطقه انتخاب‌شده رویدادی وجود ندارد';
      ...
      <table style="width:100%;border-collapse:collapse;table-layout:fixed">
        ...
        <thead>
          <tr style="font-size:calc(9px * var(--wm-panel-effective-scale, 1));font-weight:600;color:rgba(255,255,255,0.25);text-transform:uppercase;letter-spacing:0.06em">
            <th style="text-align:left;padding:0 8px 8px 0;font-weight:600">رویداد</th>

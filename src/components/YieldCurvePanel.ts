...
function renderChart(current: YieldPoint[], prior: YieldPoint[], ecbRates: Record<string, number> | null): string {
  ...
  if (allValues.length === 0) return '<div style="padding:16px;color:var(--text-dim);font-size:calc(12px * var(--wm-panel-effective-scale, 1))">داده منحنی بازده موجود نیست.</div>';
...
function renderTable(points: YieldPoint[]): string {
  ...
  const cells = points.map(p => {
    const val = p.value !== null ? `${p.value.toFixed(2)}%` : 'ناموجود';
...
function renderRatesTab(rows: RateRow[]): string {
  ...
  if (!hasAny) return '<div style="padding:16px;color:var(--text-dim);font-size:calc(12px * var(--wm-panel-effective-scale, 1))">داده نرخ‌های ECB در دسترس نیست</div>';
  ...
  return `<div style="padding:4px 0">${items}</div>
    <div style="margin-top:8px;font-size:calc(9px * var(--wm-panel-effective-scale, 1));color:var(--text-dim)">منبع: ECB</div>`;
}
...
export class YieldCurvePanel extends Panel {
  ...
  constructor() {
    super({ id: 'yield-curve', title: 'منحنی بازده و نرخ‌ها', showCount: false, infoTooltip: t('components.yieldCurve.infoTooltip') });
  ...
  public async fetchData(): Promise<boolean> {
    ...
    try {
      ...
      if (validCount === 0) {
        if (!this._hasData) this.showError('داده منحنی بازده موجود نیست', () => void this.fetchData());
        return false;
      }
      ...
      if (!this._hasData) this.showError(e instanceof Error ? e.message : 'بارگذاری منحنی بازده ناموفق بود', () => void this.fetchData());
      return false;
    }
  }
  ...
  private _render(): void {
    const tabBar = `<div style="display:flex;gap:4px;margin-bottom:6px">
      <button class="panel-tab${this._tab === 'curve' ? ' active' : ''}" data-tab="curve" style="font-size:calc(11px * var(--wm-panel-effective-scale, 1));padding:3px 10px">منحنی آمریکا</button>
      <button class="panel-tab${this._tab === 'rates' ? ' active' : ''}" data-tab="rates" style="font-size:calc(11px * var(--wm-panel-effective-scale, 1));padding:3px 10px">نرخ‌های ECB</button>
    </div>`;
    ...
    const statusBadge = isInverted
      ? `<span style="background:#e74c3c;color:#fff;font-size:calc(9px * var(--wm-panel-effective-scale, 1));font-weight:700;padding:2px 6px;border-radius:4px;letter-spacing:0.08em">معکوس</span>`
      : `<span style="background:#2ecc71;color:#000;font-size:calc(9px * var(--wm-panel-effective-scale, 1));font-weight:700;padding:2px 6px;border-radius:4px;letter-spacing:0.08em">عادی</span>`;

    const spreadHtml = spreadBps !== null
      ? `<span style="font-size:calc(11px * var(--wm-panel-effective-scale, 1));color:var(--text-dim);margin-left:10px">اختلاف بازده ۲ساله و ۱۰ساله: <span style="color:${isInverted ? '#e74c3c' : '#2ecc71'}">${escapeHtml(spreadSign + spreadBps)}bps</span></span>`
      : '';

    const ecbLegend = this._ecbRates
      ? `<span><svg width="20" height="4" style="vertical-align:middle"><line x1="0" y1="2" x2="20" y2="2" stroke="#2ecc71" stroke-width="1.5" stroke-dasharray="5,3"/></svg> اتحادیه اروپا (ECB AAA)</span>`
      : '';
    ...
    this.setSafeContent(unsafeRawHtml(`
      <div style="padding:10px 14px 6px">
        ...
        <div style="margin-top:8px;font-size:calc(9px * var(--wm-panel-effective-scale, 1));color:var(--text-dim);display:flex;gap:12px;align-items:center;flex-wrap:wrap">
          <span><svg width="20" height="4" style="vertical-align:middle"><line x1="0" y1="2" x2="20" y2="2" stroke="#3498db" stroke-width="2"/></svg> آمریکا (فعلی)</span>
          <span><svg width="20" height="4" style="vertical-align:middle"><line x1="0" y1="2" x2="20" y2="2" stroke="rgba(255,255,255,0.3)" stroke-width="1.5" stroke-dasharray="4,3"/></svg> آمریکا (قبلی)</span>
          ${ecbLegend}
          <span style="margin-left:auto">منبع: FRED / ECB</span>
        </div>

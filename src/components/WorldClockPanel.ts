...
const WORLD_CITIES: CityEntry[] = [
  { id: 'tehran', city: 'تهران', label: 'ایران', timezone: 'Asia/Tehran' },
  { id: 'new-york', city: 'نیویورک', label: 'NYSE', timezone: 'America/New_York', marketOpen: 9, marketClose: 16 },
  { id: 'chicago', city: 'شیکاگو', label: 'CME', timezone: 'America/Chicago', marketOpen: 8, marketClose: 15 },
  { id: 'sao-paulo', city: 'سائوپائولو', label: 'B3', timezone: 'America/Sao_Paulo', marketOpen: 10, marketClose: 17 },
  { id: 'london', city: 'لندن', label: 'LSE', timezone: 'Europe/London', marketOpen: 8, marketClose: 16 },
  { id: 'paris', city: 'پاریس', label: 'Euronext', timezone: 'Europe/Paris', marketOpen: 9, marketClose: 17 },
  { id: 'frankfurt', city: 'فرانکفورت', label: 'XETRA', timezone: 'Europe/Berlin', marketOpen: 9, marketClose: 17 },
  { id: 'zurich', city: 'زوریخ', label: 'SIX', timezone: 'Europe/Zurich', marketOpen: 9, marketClose: 17 },
  { id: 'moscow', city: 'مسکو', label: 'MOEX', timezone: 'Europe/Moscow', marketOpen: 10, marketClose: 18 },
  { id: 'istanbul', city: 'استانبول', label: 'BIST', timezone: 'Europe/Istanbul', marketOpen: 10, marketClose: 18 },
  { id: 'riyadh', city: 'ریاض', label: 'Tadawul', timezone: 'Asia/Riyadh', marketOpen: 10, marketClose: 15 },
  { id: 'dubai', city: 'دبی', label: 'DFM', timezone: 'Asia/Dubai', marketOpen: 10, marketClose: 14 },
  { id: 'mumbai', city: 'بمبئی', label: 'NSE', timezone: 'Asia/Kolkata', marketOpen: 9, marketClose: 15 },
  { id: 'bangkok', city: 'بانکوک', label: 'SET', timezone: 'Asia/Bangkok', marketOpen: 10, marketClose: 16 },
  { id: 'singapore', city: 'سنگاپور', label: 'SGX', timezone: 'Asia/Singapore', marketOpen: 9, marketClose: 17 },
  { id: 'hong-kong', city: 'هنگ‌کنگ', label: 'HKEX', timezone: 'Asia/Hong_Kong', marketOpen: 9, marketClose: 16 },
  { id: 'shanghai', city: 'شانگهای', label: 'SSE', timezone: 'Asia/Shanghai', marketOpen: 9, marketClose: 15 },
  { id: 'seoul', city: 'سئول', label: 'KRX', timezone: 'Asia/Seoul', marketOpen: 9, marketClose: 15 },
  { id: 'tokyo', city: 'توکیو', label: 'TSE', timezone: 'Asia/Tokyo', marketOpen: 9, marketClose: 15 },
  { id: 'sydney', city: 'سیدنی', label: 'ASX', timezone: 'Australia/Sydney', marketOpen: 10, marketClose: 16 },
  { id: 'auckland', city: 'اوکلند', label: 'NZX', timezone: 'Pacific/Auckland', marketOpen: 10, marketClose: 16 },
  { id: 'toronto', city: 'تورنتو', label: 'TSX', timezone: 'America/Toronto', marketOpen: 9, marketClose: 16 },
  { id: 'mexico-city', city: 'مکزیکوسیتی', label: 'BMV', timezone: 'America/Mexico_City', marketOpen: 8, marketClose: 15 },
  { id: 'buenos-aires', city: 'بوئنوس‌آیرس', label: 'BYMA', timezone: 'America/Argentina/Buenos_Aires', marketOpen: 11, marketClose: 17 },
  { id: 'johannesburg', city: 'ژوهانسبورگ', label: 'JSE', timezone: 'Africa/Johannesburg', marketOpen: 9, marketClose: 17 },
  { id: 'cairo', city: 'قاهره', label: 'EGX', timezone: 'Africa/Cairo', marketOpen: 10, marketClose: 14 },
  { id: 'lagos', city: 'لاگوس', label: 'NGX', timezone: 'Africa/Lagos', marketOpen: 10, marketClose: 14 },
  { id: 'los-angeles', city: 'لس‌آنجلس', label: 'اقیانوس آرام', timezone: 'America/Los_Angeles' },
  { id: 'jakarta', city: 'جاکارتا', label: 'IDX', timezone: 'Asia/Jakarta', marketOpen: 9, marketClose: 16 },
  { id: 'taipei', city: 'تایپه', label: 'TWSE', timezone: 'Asia/Taipei', marketOpen: 9, marketClose: 13 },
  { id: 'kuala-lumpur', city: 'کوالالامپور', label: 'Bursa', timezone: 'Asia/Kuala_Lumpur', marketOpen: 9, marketClose: 17 },
];

const CITY_REGIONS: { name: string; ids: string[] }[] = [
  { name: 'قاره آمریکا', ids: ['new-york', 'chicago', 'toronto', 'los-angeles', 'mexico-city', 'sao-paulo', 'buenos-aires'] },
  { name: 'اروپا', ids: ['london', 'paris', 'frankfurt', 'zurich', 'moscow', 'istanbul'] },
  { name: 'خاورمیانه و آفریقا', ids: ['tehran', 'riyadh', 'dubai', 'cairo', 'lagos', 'johannesburg'] },
  { name: 'آسیا-اقیانوسیه', ids: ['mumbai', 'bangkok', 'jakarta', 'kuala-lumpur', 'singapore', 'hong-kong', 'shanghai', 'taipei', 'seoul', 'tokyo', 'sydney', 'auckland'] },
];
...
const DEFAULT_CITIES = ['tehran', 'dubai', 'london', 'new-york', 'tokyo', 'sydney'];
...
function getTimeInZone(tz: string): {
  h: number; m: number; s: number; dayOfWeek: string; abbr: string; isWeekend: boolean;
} {
  try {
    const now = new Date();
    ...
    if (h === 24) h = 0;
    const weekdayCode = new Intl.DateTimeFormat('en-US', { timeZone: tz, weekday: 'short' }).format(now);
    return { h, m, s, dayOfWeek, abbr, isWeekend: weekdayCode === 'Sat' || weekdayCode === 'Sun' };
  } catch {
    return { h: 0, m: 0, s: 0, dayOfWeek: '', abbr: '', isWeekend: false };
  }
}

function pad2(n: number): string {
  const raw = n < 10 ? `0${n}` : `${n}`;
...
export class WorldClockPanel extends Panel {
  ...
  constructor() {
    super({ id: 'world-clock', title: 'ساعت جهانی', trackActivity: false, infoTooltip: t('components.worldClock.infoTooltip') });
    this.homeCityId = 'tehran';
    this.selectedCities = loadSelectedCities();

    this.settingsBtn = document.createElement('button');
    this.settingsBtn.className = 'wc-settings-btn';
    this.settingsBtn.textContent = '\u2699';
    this.settingsBtn.title = 'انتخاب شهرها';
  ...
  private toggleSettings(): void {
    ...
    if (this.showingSettings) {
      this.settingsBtn.textContent = '\u2713';
      this.settingsBtn.title = 'انجام شد';
  ...
  private renderClocks(): void {
    ...
    if (sorted.length === 0) {
      this.setSafeContent(
        unsafeRawHtml('<div class="wc-empty">شهری انتخاب نشده است. برای افزودن شهر روی ⚙ کلیک کنید.</div>', 'legacy Panel.setContent() migration'),
        () => this.clockRefs.clear(),
      );
      return;
    }
    ...
    for (const city of sorted) {
      const { h, m, s, dayOfWeek, abbr, isWeekend } = getTimeInZone(city.timezone);
      ...
      if (city.marketOpen !== undefined && city.marketClose !== undefined) {
        ...
        statusHtml = isOpen
          ? '<span class="wc-status open"><span class="wc-dot open"></span>باز</span>'
          : '<span class="wc-status closed"><span class="wc-dot closed"></span>بسته</span>';
      }
      ...
      html += `<div class="${rowCls.join(' ')}" data-city-id="${city.id}"><div class="wc-drag-handle" title="برای تغییر ترتیب بکشید">\u22EE</div><div class="wc-info"><div class="wc-name">${city.city}${isHome ? '<span class="wc-home-tag">\u2302</span>' : ''}</div><div class="wc-detail"><span class="wc-exchange">${city.label}</span>${statusHtml}</div></div><div class="wc-clock"><div class="wc-time">${pad2(h)}:${pad2(m)}:${pad2(s)}</div><div class="wc-tz"><div class="wc-bar-wrap"><div class="wc-bar ${isDay ? 'day' : 'night'}" style="width:${pct.toFixed(1)}%"></div></div><span>${dayOfWeek} ${abbr}</span></div></div></div>`;
    }
    html += '</div>';
  ...
  private tickClocks(): void {
    for (const refs of this.clockRefs.values()) {
      ...
      if (refs.status && city.marketOpen !== undefined && city.marketClose !== undefined) {
        const isOpen = !isWeekend && h >= city.marketOpen && h < city.marketClose;
        if (last.isOpen !== isOpen) {
          ...
          if (label && label.nodeType === Node.TEXT_NODE) {
            label.nodeValue = isOpen ? 'باز' : 'بسته';
          }
          last.isOpen = isOpen;
        }
      }

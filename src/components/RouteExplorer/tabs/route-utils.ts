...
export function formatCostDelta(addedTransitDays: number, addedCostMultiplier: number): string {
  const days = addedTransitDays > 0 ? `+${addedTransitDays} روز` : '\u2014';
...
const WAR_RISK_LABELS: Record<string, string> = {
  WAR_RISK_TIER_UNSPECIFIED: 'نامشخص',
  WAR_RISK_TIER_NORMAL: 'عادی',
  WAR_RISK_TIER_ELEVATED: 'افزایش‌یافته',
  WAR_RISK_TIER_HIGH: 'بالا',
  WAR_RISK_TIER_CRITICAL: 'بحرانی',
  WAR_RISK_TIER_WAR_ZONE: 'منطقه جنگی',
};
...
const CORRIDOR_STATUS_LABELS: Record<CorridorStatus, string> = {
  ...
  CORRIDOR_STATUS_PROPOSED: '(پیشنهادی)',
  CORRIDOR_STATUS_UNAVAILABLE: '(غیردردسترس)',
};

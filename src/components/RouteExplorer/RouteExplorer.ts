...
const TAB_LABELS: Record<ExplorerTab, string> = { 1: 'مسیر فعلی', 2: 'مسیرهای جایگزین', 3: 'مسیر زمینی', 4: 'اثرگذاری' };
...
export class RouteExplorer {
  ...
  private showLoading(): void {
    if (this.contentEl) {
      setTrustedHtml(this.contentEl, trustedHtml('<div class="re-content__loading">در حال بارگذاری داده مسیر…</div>', "legacy direct innerHTML migration"));
    }
  }

  private showError(): void {
    if (this.contentEl) {
      setTrustedHtml(this.contentEl, trustedHtml('<div class="re-content__error">بارگذاری داده مسیر ناموفق بود. دوباره تلاش کنید.</div>', "legacy direct innerHTML migration"));
    }
  }
  ...
  private renderFreeGate(): void {
    ...
    if (this.contentEl) {
      setTrustedHtml(this.contentEl, trustedHtml('<div class="re-content__gate">' +
        '<h3>دسترسی به اطلاعات مسیر</h3>' +
        '<ul><li>مسیر فعلی همراه با ریسک گلوگاه</li><li>مسیرهای جایگزین رتبه‌بندی‌شده</li><li>گزینه‌های کریدور زمینی</li></ul>' +
        checkoutConsentHtml(WEB_APP_ORIGIN) +
        '<button class="re-content__upgrade" type="button">ارتقا به نسخه PRO</button>' +
  ...
  private buildRoot(): HTMLDivElement {
    ...
    root.setAttribute('aria-label', 'کاوشگر مسیر — برنامه‌ریزی حمل‌ونقل');
  ...
  private buildQueryBar(): HTMLDivElement {
    ...
    back.setAttribute('aria-label', 'بستن کاوشگر مسیر');

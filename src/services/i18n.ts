...
import faShellTranslation from '../locales/fa.shell.json';
...
const localeModules = import.meta.glob<TranslationDictionary>(
  ['../locales/*.json', '!../locales/*.shell.json'],
  { import: 'default' },
);
...
export async function initI18n(): Promise<void> {
  ...
  // - wmExplicit reads the explicit-choice key. In this Persian-localized build,
  //   an unset preference defaults to Farsi; explicit user choices still persist. This replaces the default
  ...
  detector.addDetector({
    name: 'wmExplicit',
    lookup: () => {
      try { return localStorage.getItem(EXPLICIT_LOCALE_KEY) || 'fa'; }
      catch { return 'fa'; }
    },
  ...
  await i18next
    ...
    .init({
      resources: {
        ...
        fa: { translation: faShellTranslation as TranslationDictionary },
      },
...
export function getLocale(): string {
  ...
  const map: Record<string, string> = { en: 'en-US', bg: 'bg-BG', cs: 'cs-CZ', el: 'el-GR', fa: 'fa-IR-u-ca-persian', zh: 'zh-CN', pt: 'pt-BR', ja: 'ja-JP', ko: 'ko-KR', ro: 'ro-RO', tr: 'tr-TR', th: 'th-TH', vi: 'vi-VN', hi: 'hi-IN' };

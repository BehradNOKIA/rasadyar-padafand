...
# =============================================================================
# Multi-stage build:
#   builder       — installs deps, builds canonical-source artifacts, compiles API handlers
...
# Copy canonical source tree.
COPY . .

# Validate the committed attribution manifest against authored source before
# any Docker-only build step writes generated JavaScript into api/.
# This is deliberately a read-only check; never regenerate the manifest here.
RUN node scripts/source-attribution.mjs

# Generated inventory modules are intentionally untracked. Recreate them in
# the clean image context before the application build.
...
# All source-introspection work must run before docker/build-handlers.mjs.
# build-handlers writes bundled api/**/*.js next to authored api/**/*.ts files;
# source-attribution intentionally scans api/ and .js files, so bundling earlier
# would make Docker validate a mutated source tree instead of canonical source.
#
...
# Runtime packaging must happen only after every source-attribution/source-scan
# dependent build step has completed.
